...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  //Funcion cmprobar los campos del regsitro
  ...
  if(fullName.isEmpty && username.isEmpty && email.isEmpty && password.isEmpty && confirmPassword.isEmpty){
    _showErrorSnackBar('All fields are empty.');
    return;
  }

  else if (username.isEmpty || username.length > 20) {
    _showErrorSnackBar('Username must be between 1 and 20 characters long.');
    return;
  }

  else if (password.isEmpty || password.length < 8 || password.length > 20) {
    _showErrorSnackBar('Password must be between 8 and 20 characters long.');
    return;
  }
  ...
  else if (fullName.isEmpty || fullName.length > 40) {
    _showErrorSnackBar('Full name must be between 1 and 40 characters long.');
    return;
  }

  else if (email.isEmpty || email.length > 40 || !RegExp(r'^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$').hasMatch(email)) {
